...
const addExclamation = (arr) => {
  let strings = []
  arr.forEach(addExclamation => { strings.push(addExclamation + '!') })
  return strings;
  // Solution code here...
};
...
const allUpperCase = (arr) => {
  ...
  arr.forEach(allUpperCase => {
    strings.push(allUpperCase.toUpperCase()); });
    return strings;
...
const speaker = (words, callback) => {
  // Solution code here...
  let strings=[];
  words.forEach(speaker => {
    strings.push(callback(speaker));
  })
  return strings
...
const createList = (availableItems) => {
 ...
 availableItems.forEach(element => {
   if (element.available=== true) {
     newinventory.push(element.name);
